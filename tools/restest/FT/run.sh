...
while [ $ATTEMPT -le $MAX_RUNS ]; do
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        echo "INFO: Attempt #$ATTEMPT FT succeeded."
        ANY_SUCCESS=1
    else
        echo "ERROR: Attempt #$ATTEMPT of FT failed with exit code $EXIT_CODE."
    fi
...
if [ $ANY_SUCCESS -eq 1 ]; then
    echo "INFO: At least one attempt succeeded for FT. Returning 0."
    exit 0
else
    echo "ERROR: All $MAX_RUNS attempts failed for FT. Returning last exit code: $LAST_EXIT_CODE."
    exit $LAST_EXIT_CODE
fi
